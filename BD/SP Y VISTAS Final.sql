...
AS
    SELECT idempl,DNI, Nom, ApelPate, ApelMate, RUC, Email, Telf, Cel, FecNac, GrupSang, EstCiv, ConLab, CarnAseg, Refe, Leye, FecIng, FecNom, DatCony, UbigActu AS 'Direccion', Est,
    ...
    FROM Empleado
        LEFT OUTER JOIN Familiares ON Empleado.idEmpl = Familiares.Empleado_idEmpl
        LEFT OUTER JOIN Emergencia ON Empleado.idEmpl = Emergencia.Empleado_idEmpl
        LEFT OUTER JOIN EstudioBasicos ON Empleado.idEmpl = EstudioBasicos.Empleado_idEmpl
        LEFT OUTER JOIN EstudiosSuperior ON Empleado.idEmpl = EstudiosSuperior.Empleado_idEmpl
        LEFT OUTER JOIN Ubigeo ON Empleado.Ubigeo_CodUbi = Ubigeo.CodUbi
        LEFT OUTER JOIN DetalleDialecto ON Empleado.idEmpl = DetalleDialecto.Empleado_idEmpl
...
AS
    BEGIN
        IF(SELECT Empleado.DNI FROM Empleado WHERE Empleado.DNI = @DNI) IS NOT NULL
             SELECT 'DNI YA EXISTE' AS 'RESULTADO'
        ELSE
